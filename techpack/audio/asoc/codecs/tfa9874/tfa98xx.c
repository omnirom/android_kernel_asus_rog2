...
/* ASUS_BSP Paul +++ */
static DEFINE_MUTEX(tfa98xx_mixer_ctl_lock);
static int tfa98xx_mixer_ctl_dev_count = 0;
/* ASUS_BSP Paul --- */
...
#endif
	mutex_lock(&tfa98xx_mixer_ctl_lock);
	tfa98xx_mixer_ctl_dev_count++;
	if (tfa98xx_mixer_ctl_dev_count == 2)
		tfa98xx_create_controls(tfa98xx);
	mutex_unlock(&tfa98xx_mixer_ctl_lock);
	/* ASUS_BSP Paul --- */
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK){
			dev_dbg(&tfa98xx->i2c->dev,"tfa98xx_mute: tfa98xx->pstream mute\n");
			mutex_lock(&tfa98xx_mutex);
			tfa98xx_sync_count = 0;
			mutex_unlock(&tfa98xx_mutex);
...
#ifdef TFA9874_NONDSP_STEREO
		{
			dev_dbg(&tfa98xx->i2c->dev,"tfa98xx_mute: tfa9874_nondsp_stereo  tfa98xx->pstream unmute\n");
...
#else
		{
			...
			if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING){
				dev_dbg(&tfa98xx->i2c->dev,"tfa98xx_mute: start wordqueue init_work\n");
				queue_delayed_work(tfa98xx->tfa98xx_wq,
								   &tfa98xx->init_work, 0);
			}
		}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	dev_err(codec->dev, "tfa98xx_probe");
	pr_debug("\n");
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		case 0x74: /* tfa9874 */
			...
			tfa98xx->flags |= TFA98XX_FLAG_CALIBRATION_CTL;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS; /* ASUS_BSP Paul +++ */
			break;
		case 0x88: /* tfa9888 */
	...
	dev_err(&i2c->dev, " register TFA98xx: %d\n", ret);
