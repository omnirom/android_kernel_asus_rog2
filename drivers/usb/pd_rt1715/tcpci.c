...
#ifdef CONFIG_TYPEC_CAP_A2C_C2C
	tcp_noti.typec_state.a2c_cable = tcpc->typec_a2c_cable;

	tcpc->typec_a2c_cable_final = false;
	if (tcpc->typec_attach_new == TYPEC_ATTACHED_SNK ||
		tcpc->typec_attach_new == TYPEC_ATTACHED_CUSTOM_SRC) {
		tcpc->typec_a2c_cable_final = tcpc->typec_a2c_cable;

		if (tcp_noti.typec_state.a2c_cable)
			TCPC_INFO("** A2C_CABLE\r\n");
		else
			TCPC_INFO("** C2C_CABLE\r\n");
	}

#ifdef CONFIG_TYPC_CAP_NORP_SRC
	if (tcpc->typec_attach_new == TYPEC_ATTACHED_NORP_SRC) {
		tcpc->typec_a2c_cable_final == true;
		tcp_noti.typec_state.a2c_cable = true;
	}
