...
static int ene_6k582_read_bytes(struct i2c_client *client, short addr, char *data)
{
	...
	pr_debug("[AURA_POGO] ene_6k582_read_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2]);
...
static int ene_6k582_write_bytes(struct i2c_client *client, short addr, char value)
{
	...
	pr_debug("[AURA_POGO] ene_6k582_write_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x, value : 0x%x\n", (addr & 0xFFFF), buf[1], buf[2], value);
	err = i2c_write_bytes(client, buf, 3);	//set register address
	if (err !=1)
		printk("[AURA_POGO] i2c_write_bytes:err %d\n", err);
...
static ssize_t red_pwm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret)
		return count;
	pr_debug("[AURA_POGO] %s reg_val=%d \n",__func__,reg_val);
...
static ssize_t apply_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(g_panel_state==0){
		...
		printk("[AURA_POGO] resume for aura on when panel off\n");
		mutex_lock(&g_pdata->ene_mutex);
		err = ene_6k582_read_bytes(client, 0x8453, data_r);
		if (err != 2){
			printk("[AURA_POGO] sleep_show 1st read before write:err %d\n", err);
		}
		mutex_unlock(&g_pdata->ene_mutex);
		if(data_r[0]==0xAA){
			printk("[AURA_POGO] ene_6k582_resume  ok, after read 1st\n");
			return count;
		}
		sleep_val = 0xAA;
		mutex_lock(&g_pdata->ene_mutex);
		...
		if (err !=1){
			printk("[AURA_POGO] ene_6k582_write_bytes 0X8453-0xAA :err %d\n", err);
		}
		err = ene_6k582_read_bytes(client, 0x8453, data_r);
		if (err != 2){
		...
		mutex_unlock(&g_pdata->ene_mutex);
		if(data_r[0]==0xAA){
			printk("[AURA_POGO] ene_6k582_resume ok after first write and check\n");
			return count;
		}else{
				...
				err = ene_6k582_read_bytes(client, 0x8453, data_r);
				if (err != 2){
					printk("[AURA_POGO] sleep_show 2nd read before write:err %d\n", err);
				}
				mutex_unlock(&g_pdata->ene_mutex);
				if(data_r[0]==0xAA){
					printk("[AURA_POGO] ene_6k582_resume  ok, after read 2nd\n");
					return count;
				}
				sleep_val = 0xAA;
				mutex_lock(&g_pdata->ene_mutex);
				...
				mutex_unlock(&g_pdata->ene_mutex);
				if(data_r[0]==0xAA){
					printk("[AURA_POGO] ene_6k582_resume ok after 2nd write and check\n");
						return count;
				}else{
					...
					mutex_unlock(&g_pdata->ene_mutex);
					if(data_r[0]==0xAA){
						printk("[AURA_POGO] ene_6k582_resume  ok, after read 3rd\n");
						return count;
					}
					sleep_val = 0xAA;
					mutex_lock(&g_pdata->ene_mutex);
					...
					mutex_unlock(&g_pdata->ene_mutex);
					if(data_r[0]==0xAA){
						printk("[AURA_POGO] ene_6k582_resume ok after 3rd write and check\n");
						return count;
					}else{
						printk("[AURA_POGO] ene_6k582_resume fail after retry three times\n");
					}
...
static ssize_t update_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (err)
		return count;
	printk("[AURA_POGO]update_state_store val=%d \n",val);
...
static int ene_6k582_fb_callback(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	switch (blank) {
		case MSM_DRM_BLANK_POWERDOWN:
			if(g_update_state==1){
				printk("[AURA_POGO]it is updating, not sleep\n");
				return NOTIFY_OK;
			}
			if(platform_data->current_mode == 0){
				g_panel_state=0;
				printk("[AURA_POGO] ene_6k582_suspend it is mode 0,suspend. \n");
				sleep_val = 0xBE;
				mutex_lock(&g_pdata->ene_mutex);
				err = ene_6k582_write_bytes(client, 0x8453, sleep_val);
				if (err !=1)
					printk("[AURA_POGO] ene_6k582_write_bytes 0x8453-0xBE:err %d\n", err);
				mutex_unlock(&g_pdata->ene_mutex);
			}
			break;
		case MSM_DRM_BLANK_UNBLANK:
			g_panel_state=1;
			if(gDongleType!=2){
				printk("[AURA_POGO] ene_6k582_resume return for station removed\n");
				return NOTIFY_OK;
			}
			if(platform_data->current_mode == 0){
				printk("[AURA_POGO] ene_6k582_resume it is mode 0,resume\n");
				mutex_lock(&g_pdata->ene_mutex);
				err = ene_6k582_read_bytes(client, 0x8453, data_r);
				if (err != 2){
					printk("[AURA_POGO] sleep_show 1st read before write:err %d\n", err);
...
int ene_6k582_suspend(struct device *dev)
{
	...
	if(g_sleep==1){
		printk("[AURA_POGO] ene_6k582_suspend not sleep,  return\n");
		return 0;
	}
	if(g_delay_time!=0){
...
int ene_6k582_resume(struct device *dev)
{
	...
	struct ene_6k582_platform_data *platform_data = i2c_get_clientdata(client);
	if(g_sleep==1){
		printk("[AURA_POGO] ene_6k582_resume not sleep, return\n");
		return 0;
	}
