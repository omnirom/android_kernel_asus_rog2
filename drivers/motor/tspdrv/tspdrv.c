...
//extern bool bEnable_Immrvib;
extern u8 bEnable_Vib;
...
static int __init tspdrv_init(void)
{
    ...
    DbgOutInfo(("call VibeOSKernelLinuxInitTimer.\n"));
    VibeOSKernelLinuxInitTimer();
    DbgOutInfo(("call ResetOutputData.\n"));
    ResetOutputData();

    /* Get and concatenate device name and initialize data buffer */
    g_cchDeviceName = 0;
    DbgOutInfo(("call  Get and concatenate device name.\n"));
    for (i=0; i<NUM_ACTUATORS; i++)
    {
        char *szName = g_szDeviceName + g_cchDeviceName;
		DbgOutInfo(("call ImmVibeSPI_Device_GetName.\n"));
...
static ssize_t write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    */
    ...
    {
        /* Failed to copy all the data, exit */
        DbgOutErr(("copy_from_user failed.\n"));
        return -EIO;
    }
    ...
    {
        DbgOutErr(("SaveOutputData failed.\n"));
        return -EIO;
    }

    /* Start the timer after receiving new output force */
    g_bIsPlaying = true;
	//if(!bEnable_Immrvib) return count;
    //DbgOutInfo(("%s:call VibeOSKernelLinuxStartTimer\n", __func__));
...
#endif
{
    switch (cmd)
    {
        ...
        case TSPDRV_ENABLE_AMP:
			/*
			if(!bEnable_Immrvib){
				DbgOutInfo(("tspdrv: %s:TSPDRV_ENABLE_AMP:bEnable_Immrvib=%s\n", __func__,(bEnable_Immrvib==true)?"true":"false" ));
				return -1;
			}
			*/
			if(bEnable_Vib==0x01 && arg!=0){//only enabel bot_vib
				DbgOutInfo(("tspdrv: %s:TSPDRV_ENABLE_AMP:Top_VIB disable:(bEnable_Vib=0x%02x,index=%d)\n", __func__,bEnable_Vib, arg));
				return -1;
			}
			if(bEnable_Vib==0x02 && arg!=1){//only enabel top_vib
				DbgOutInfo(("tspdrv: %s:TSPDRV_ENABLE_AMP:Bot_VIB disable:(bEnable_Vib=0x%02x,index=%d)\n", __func__,bEnable_Vib, arg));
				return -1;
			}
			//DbgOutInfo(("tspdrv: %s:TSPDRV_ENABLE_AMP:call ImmVibeSPI_ForceOut_AmpEnable\n", __func__ ));
        ...
        /*
			if(!bEnable_Immrvib){
				DbgOutInfo(("tspdrv: %s:TSPDRV_DISABLE_AMP:bEnable_Immrvib=%s\n", __func__,(bEnable_Immrvib==true)?"true":"false" ));
				return -1;
			}
		*/
			//DbgOutInfo(("tspdrv: %s:TSPDRV_DISABLE_AMP:call ImmVibeSPI_ForceOut_AmpEnable\n", __func__ ));
        ...
        case TSPDRV_SET_RUNTIME_RECORD_FLAG:
            {
                ...
                if (nRecordFlag) {
                    int i;
                    for (i=0; i<NUM_ACTUATORS; i++) {
                        DbgRecorderReset((i));TSPDRV_ENABLE_AMP
                    }
                }
                break;
            }
