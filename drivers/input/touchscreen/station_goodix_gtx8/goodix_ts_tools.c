...
#define GOODIX_TOOLS_NAME		"station_gtp_tools"
					...
					unsigned long arg)
{
	...
	case GTP_IRQ_ENABLE:
		if (arg == 1) {
			station_goodix_ts_irq_enable(dev->ts_core, true);
			mutex_lock(&dev->mutex);
			dev->ops_mode |= IRQ_FALG;
			mutex_unlock(&dev->mutex);
			ts_info("IRQ enabled");
		} else if (arg == 0) {
			station_goodix_ts_irq_enable(dev->ts_core, false);
			mutex_lock(&dev->mutex);
			dev->ops_mode &= ~IRQ_FALG;
			mutex_unlock(&dev->mutex);
	...
	case GTP_ESD_ENABLE:
		if (arg == 0)
			station_goodix_ts_blocking_notify(NOTIFY_ESD_OFF, NULL);
		else
			station_goodix_ts_blocking_notify(NOTIFY_ESD_ON, NULL);
		break;
	case GTP_DEV_RESET:
		hw_ops->reset(ts_dev);
		break;
...
static int goodix_tools_open(struct inode *inode, struct file *filp)
{
	...
	ret = station_goodix_register_ext_module(&goodix_tools_dev->module);
	if (ret) {
...
static int goodix_tools_release(struct inode *inode, struct file *filp)
{
	...
	ret = station_goodix_unregister_ext_module(&goodix_tools_dev->module);
	return ret;
}
