...
#define TS_DRIVER_NAME			"station_gtx8"
		...
		struct goodix_ts_board_data *board_data)
{
	...
	ts_info("swap-axis: %d x2x: %d y2y : %d", board_data->swap_axis, board_data->x2x, board_data->y2y);
...
int check_dt(void)
{
	...
	count = of_count_phandle_with_args(g_np, "panel", NULL);
	if (count <= 0)
		return 0;

	for (i = 0; i < count; i++) {
		node = of_parse_phandle(g_np, "panel", i);
		panel = of_drm_find_panel(node);
		of_node_put(node);
		if (!IS_ERR(panel)) {
			active_panel_asus_station = panel;
			return 1;
		}
	}
		...
		struct goodix_ts_config *config)
{
	...
	struct goodix_ts_cmd ts_cmd;
	ts_info("_do_goodix_send_config start");
...
exit:
        ts_info("_do_goodix_send_config end");
	return r;
}
