...
//                        ts_info("input report g_id_fod:%d id:%d x:%d,y:%d,p:%d", g_id_fod, id, coords->x, coords->y,coords->p);
                    pre_fin |= 1 << i;
                    id = (++coords)->id;
                    ts_info("Touch ignore g_notify_fod:%d disable_fod:%d", atomic_read(&g_notify_fod), core_data->disable_fod);
                    continue;
                } else if (fod_press == (1<<i)){ 
					pre_fin |= 1 << i;
					id = (++coords)->id;
                    ts_info("Touch ignore fod_press:%d", fod_press);
					continue;
                }
            } else if (fod_press == (1<<i)){ 
					pre_fin |= 1 << i;
					id = (++coords)->id;
                    ts_info("Touch ignore g_notify_fod:%d disable_fod:%d fod_press:%d", atomic_read(&g_notify_fod), core_data->disable_fod, fod_press);
					continue;
            }
            mutex_lock(&dev->mutex);
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if(core_data->debug ) {
		...
		if (!ext_module->funcs->tool_irq_event) {
            ts_info("!ext_module->funcs->tool_irq_event\n");
		    continue;
        }
	...
	list_for_each_entry(ext_module, &goodix_modules.head, list) {
		if (!ext_module->funcs->irq_event) {
            ts_info("!ext_module->funcs->irq_event\n");
			continue;
        }
...
enum hrtimer_restart goodix_debug_work(struct hrtimer *timer)
{
    if((gts_core_data->gsx_gesture != NULL) && (gts_core_data != NULL)) {
        ts_info("game_cfg_set = %d, charge_mode = %d, glove_mode = %d, zen_motion = %d, dclick = %d, swipeup = %d, aod_enable = %d",
                                                                                     atomic_read(&gts_core_data->game_cfg_set),
                                                                                     atomic_read(&gts_core_data->charge_mode),
                                                                                     atomic_read(&gts_core_data->glove_mode),
                                                                                     atomic_read(&gts_core_data->gsx_gesture->zen_motion),
                                                                                     atomic_read(&gts_core_data->gsx_gesture->dclick),
                                                                                     atomic_read(&gts_core_data->gsx_gesture->swipeup),
                                                                                     atomic_read(&gts_core_data->gsx_gesture->aod_enable));
        ts_info("game config check:%d.%d.%d.%d.%d.%d.%d", game_cfg[1]|game_cfg[2], game_cfg[3]|game_cfg[4], game_cfg[5]|game_cfg[6], game_cfg[7]|game_cfg[8], game_cfg[9], game_cfg[10], game_cfg[11]);
    }

    hrtimer_forward(timer,timer->base->get_time(),dtime);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
    ...
    hrtimer_init(&core_data->debug_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    core_data->debug_timer.function = &goodix_debug_work;
    dtime = ktime_set(600, 0);
    hrtimer_start(&core_data->debug_timer, dtime, HRTIMER_MODE_REL);
	//ASUS_BSP Beryl ---
out:
	if (r!=0) {
