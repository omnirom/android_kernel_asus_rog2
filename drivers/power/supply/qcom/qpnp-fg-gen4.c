...
extern int hid_vote_register(char *name);
extern int hid_vote_unregister(int id, char *name);
...
//ASUS BSP Austin_T : Read profile 4P35V first ---
	...
	g_fv_setting = fg->bp.float_volt_uv;
	printk(KERN_ERR "%s. fg->bp.float_volt_uv : %d\n", __func__, fg->bp.float_volt_uv);
...
static int fg_gen4_charge_full_update(struct fg_dev *fg)
{
	...
	} else if ((msoc_raw <= recharge_soc /*|| !fg->charge_done*/) //ASUS_BSP remove charge_done
			&& fg->charge_full) {
		if (chip->dt.linearize_soc) {
...
static irqreturn_t fg_vbatt_low_irq_handler(int irq, void *data)
{
	...
	if (rc < 0)
		return IRQ_HANDLED;
	BAT_DBG("%s. triggered vbatt_mv: %d msoc_raw:%d\n",__func__, vbatt_mv, msoc_raw);
	...
	/* workaround for false alarm of vbatt_low_irq_handler */
	if(vbatt_mv_last >= chip->dt.cutoff_volt_mv && vbatt_mv < chip->dt.cutoff_volt_mv) {
		vbatt_mv_last = vbatt_mv;
		return IRQ_HANDLED;
	}
	vbatt_mv_last = vbatt_mv;
	/* workaround for false alarm of vbatt_low_irq_handler */
...
static int fg_gen4_parse_dt(struct fg_gen4_chip *chip)
{
	...
	printk(KERN_ERR "%s dt.sys_term_curr_ma : %d\n", __func__, chip->dt.sys_term_curr_ma);
...
{
	bat_policy_proc_file = proc_create(bat_policy_PROC_FILE, 0777, NULL, &bat_policy_fops);
...
static int batt_safety_csc_restore(void){
	BAT_DBG("For YODA, unsupport csc data restore from SD\n",__FUNCTION__);
	return 0;
	#if 0
	int rc = 0;
	struct CYCLE_COUNT_DATA buf;
...
static int batt_safety_csc_getcyclecount(void){
	...
	#if 0 //For YODA, no data need to be saved in SD
	rc = file_op(BAT_CYCLE_SD_FILE_NAME, CYCLE_COUNT_DATA_OFFSET,
		(char *)&buf, sizeof(char)*30, FILE_OP_WRITE);
	if (rc<0)
		pr_err("%s:Write file:%s err!\n", __FUNCTION__, BAT_CYCLE_SD_FILE_NAME);
