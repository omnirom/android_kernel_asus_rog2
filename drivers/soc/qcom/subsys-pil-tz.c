...
// MODULE_PARM_DESC(do_antenna_switch, "Switch VREG_L16A_2P7 flag");
static struct antenna_switch_vreg priv_switch_vreg = { NULL, "vdd-2.7-antenna", 2700000, 2700000, 0, false};
//ASUS_BSP --- JACK "add for the antenna switch power (LDO16A)"
...
static int pil_tz_driver_probe(struct platform_device *pdev)
{
	...
	//ASUS_BSP +++ JACK "add for the antenna switch power (LDO16A)"
	if(!strncmp(d->desc.name,"modem",strlen(d->desc.name))){
